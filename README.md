...
- 실제 업무 물리 테이블은 사용자가 입력한 `physical_target.db_nm`과 `physical_target.target_namespace_map`에 따라 생성한다.
...
- skill 구조와 필수 파일 존재 여부
- `SKILL.md` frontmatter
- `agents/openai.yaml` YAML 파싱과 `allow_implicit_invocation: false`
- 신규/기존 초기 입력 템플릿 YAML block 파싱
- `assets/execution-context.template.yaml` 파싱
- Execution Context nested canonical schema 필수 path
- legacy flat alias가 template root에 남아 있지 않은지 여부
...
[OK] required assets
[OK] initial-context-new-project.template.md YAML block
[OK] initial-context-existing-project.template.md YAML block
[OK] execution-context.template.yaml
[OK] execution-context.template.yaml canonical paths
...
현재 배포 버전은 안전을 위해 `agents/openai.yaml`에서 `allow_implicit_invocation: false`를 사용한다.
가장 확실한 호출 방식은 skill 이름을 직접 호출하는 것이다.
...
아래처럼 자연어로 표준화 DDL 작업을 요청할 수는 있다.
다만 현재 정책상 자연어 요청만으로 skill이 자동 호출된다고 가정하지 않는다.

```text
표준 단어/용어/도메인 사전을 기준으로 아래 한글 테이블 정의를 표준화하고 DDL preview를 만들어줘.
```

일부 runtime에서는 관련 skill을 추천하거나 사용자가 수동으로 선택할 수 있다.
그 경우에도 안전하게 진행하려면 `$db-standard-ddl-workflow` 명시 호출을 권장한다.
...
1. 요청 database와 `physical_target.db_nm` 일치 확인
2. 요청 namespace와 `physical_target.target_namespace_map` 일치 확인
...
- `run_control.run_mode=preview`: SELECT / preview SQL만 생성하고 쓰기 실행 금지
- `run_control.run_mode=approval`: preview SQL과 실행 순서를 제시하고 승인 대기
- `run_control.run_mode=execute`: 사용자 명시 승인, `run_control.write_execution_enabled=true`, 실제 실행 도구가 모두 있을 때만 실행
