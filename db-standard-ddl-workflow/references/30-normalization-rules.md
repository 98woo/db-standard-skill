...
이 문서는 한글 테이블명 / 컬럼명을 canonical word sequence로 바꾸는 규칙을 정의한다.
...
3. 연속 공백을 1개 공백으로 정리
4. 파생어 / 복합어 정규화 적용
5. exact term / exact word 조회
6. miss 시 synonym / prohibited-word 조회
7. canonical `word_nm` 으로 치환
8. 최종 word sequence 확정
...
2. 단어 exact match
3. synonym match
4. prohibited-word match
5. 신규 등록 검토

적용 원칙:

- 위 1~4 중 하나라도 단일 canonical 후보로 확정되면 신규 등록을 하지 않는다.
- synonym / prohibited-word는 원 입력을 유지하는 규칙이 아니라 canonical `word_nm`으로 치환하는 규칙이다.
